        ...
        gint h )
    {
        ...
        if( d.isBase() || d.isEventBox())
        {
            ...
            */
            ...
            {
                Style::instance().renderTooltipBackground( context, x, y, w, h, StyleOptions() );
                return;
            }
            ...
            if( GtkWidget* parent = Gtk::gtk_parent_combobox_entry( widget ) )
            {
                ...
                */
                ...
                if( Gtk::gtk_widget_layout_is_reversed( widget ) )
                {
                    ...
                    tiles &= ~TileSet::Left;
                    Style::instance().renderHole( context, x-7, y, w+7, h, options, data, tiles );

                } else {

                    // hide right part and increase width
                    tiles &= ~TileSet::Right;
                    Style::instance().renderHole( context, x, y, w+7, h, options, data, tiles );
...
// TODO: check whether still necessary with gtk+3 and reimplement
...
//                 }
                ...
                // render hole
                const AnimationData data( Style::instance().animations().widgetStateEngine().get( widget, options, AnimationHover|AnimationFocus, AnimationFocus ) );
                Style::instance().renderHole( context, x, y, w, h, options, data, tiles );
        ...
        StyleWrapper::parentClass()->draw_flat_box(
            style, context, state,
        ...
        if( d.isInfoBar() )
        {
            ...
            if( GTK_IS_MENU( widget ) && gtk_menu_get_tearoff_state( GTK_MENU( widget ) ) )
            {
                    ...
                    Style::instance().settings().applicationName().isOpenOffice() )
                {
                    ...
                    GdkWindow* window( gtk_widget_get_window( widget ) );
                    Style::instance().renderMenuBackground( context, window, x, y, w, h, options );
...
//                 }
                ...
                GdkWindow* window( gtk_widget_get_window( widget ) );
                if( !Style::instance().renderMenuBackground( context, window, x, y, w, h, options ) )
                { options &= ~Round; }
            ...
            GdkWindow* window( gtk_widget_get_window( widget ) );
            Style::instance().renderMenuItemRect( context, window, widget, x, y, w, h, options, data );
            ...
            if( vertical ) {
                ...
                if( d.isTrough() ) Style::instance().renderSliderGroove( context, x, y + offset, w, h - 2*offset, Vertical );
                else if( d.isTroughLower() ) Style::instance().renderSliderGroove( context, x, y + offset, w, h, Vertical );
                else if( d.isTroughUpper() ) Style::instance().renderSliderGroove( context, x, y, w, h - offset, Vertical );

            } else {

                // TODO: calculate this value from the style "slider-width" property
                const int offset( 6 );
                if( d.isTrough() ) Style::instance().renderSliderGroove( context, x + offset, y, w - 2*offset, h, StyleOptions() );
                else if( d.isTroughLower() ) Style::instance().renderSliderGroove( context, x + offset, y, w, h, StyleOptions() );
                else if( d.isTroughUpper() ) Style::instance().renderSliderGroove( context, x, y, w - offset, h, StyleOptions() );
            ...
            if( GTK_IS_PROGRESS_BAR( widget ) )
            {
                    ...
                    !Style::instance().settings().applicationName().isOpenOffice() )
                {
                    ...
                    Not sure why
                    */
                    StyleWrapper::parentClass()->draw_box( style, context, state, shadow, widget, detail, x, y, w, h );
                ...
                Style::instance().renderScrollBarHole( context, x, y+1, w-1, h-1, Vertical );
                ...
                Style::instance().renderScrollBarHole( context, x+1, y, w-2, h-1, StyleOptions() );
            ...
            if( Style::instance().settings().applicationName().isOpenOffice() )
            {
                ...
                } else Style::instance().fill( context, x, y, w, h, background );
            ...
            if( Gtk::gtk_widget_layout_is_reversed( widget ) )
            {
                ...
                {
                    GdkWindow* window( gtk_widget_get_window( widget ) );
                    Style::instance().renderHoleBackground( context, window, x-5, y-1, w+6, h+2, tiles );
                }

                // shrink spinbox entry hole by 3px on right side
                x += Style::Entry_SideMargin;
                w -= Style::Entry_SideMargin;
                Style::instance().renderHole( context, x-1, y-1, w+6, h+2, options, data, tiles );
                ...
                // shrink spinbox entry hole by 3px on right side
                w -= Style::Entry_SideMargin;
                Style::instance().renderHole( context, x-5, y-1, w+6, h+2, options, data, tiles );
...
//                 }
            ...
            Style::instance().renderMenuBackground( context, parentWindow, allocation.x, allocation.y, allocation.width, allocation.height, options );
            Style::instance().drawFloatFrame( context, allocation.x, allocation.y, allocation.width, allocation.height, options );
...
//             }
            ...
            if( GtkWidget* parent = Gtk::gtk_parent_combobox_entry( widget ) )
            {
                ...
                if( Style::instance().settings().applicationName().isOpenOffice() )
                {
                    ...
                    if( d.isEntry() )
                    {
                        ...
                        Style::instance().renderHole( context, x-1, y-1, w+2, h+2, options );

                    } else {

                        Style::instance().renderHole( context, x, y, w, h, options );
                    ...
                    {
                        GdkWindow* window( gtk_widget_get_window( parent ) );
                        Style::instance().renderHoleBackground( context, window, x, y, w, h );
                    }
            ...
            if( !Style::instance().settings().applicationName().isOpenOffice() )
            {
                ...
                GdkWindow* window( gtk_widget_get_window( parent ) );
                Style::instance().renderWindowBackground( context, window, x-4, y-4, w+8, h+8 );
            ...
            // calendar header
            GdkWindow* window( gtk_widget_get_window( parent ) );
            Style::instance().fill( context, x-2, y-2, w+4, h+4, Gtk::gdk_get_color( style->base[state] ) );
            Style::instance().renderWindowBackground( context, window, widget, x+2, y+2, w-4, h-6 );

            StyleOptions options( Blend );
            options |= NoFill;
            Style::instance().renderSlab( context, x-2, y-2, w+4, h+2, options );
            ...
            if( Gtk::gtk_widget_layout_is_reversed( widget ) )
            {
                ...
                tiles &= ~TileSet::Left;
                Style::instance().renderButtonSlab( context, x-10, y, w+10, h, options, data, tiles );

            } else {

                tiles &= ~TileSet::Right;
                Style::instance().renderButtonSlab( context, x, y, w+10, h, options, data, tiles );
            ...
            StyleOptions options;
            Style::instance().renderProgressBarHole( context, x-2, y, w+4, h, options);
        ...
        GtkExpanderStyle expander_style )
    {
        ...
        */
        ...
        if( Style::instance().settings().viewDrawTriangularExpander() )
        {
            ...
            const AnimationData data( Style::instance().animations().widgetStateEngine().get( widget, options, AnimationHover ) );
            Style::instance().renderTreeExpander( context, x-3, y-5, 10, 10, expander_style, options, data, role );
